<template>
  <div class="shadow flex flex-col bg-white text-12px overflow-hidden w-180px min-w-180px h-full">
    <header class="px-8px h-22px flex-shrink-0 flex items-center text-12px font-bold text-#333 b-b-1 b-solid b-#ccc">
      棋谱序列
    ...
    <div ref="scrollRef" class="flex-1 h-full overflow-y-auto">
      <ul ref="recordRef" class="text-center">
        <li :class="`b-b ${isStart ? 'bg-red/20 font-bold' : ''}`" @click="store.readRecord(-1)">
          ===棋局开始===
        </li>
        <li v-for="([item1, item2], index) in list" class="relative flex-center gap-2 b-b" :key="index">
          <span @click="readRecord(index, 0)" class="w-54% text-left pl-3">
            <span>{{ index < 9 ? '&nbsp;' : '' }}</span>
                <span>{{ index + 1 }}.</span>
                <span :class="`hover:bg-red/10 ${getAcriveStyle(index, 0)}`">
                  {{ item1.name }}
                </span>
            </span>
            <span @click="readRecord(index, 1)" class="flex-1 text-left">
              <span :class="`hover:bg-red/10 ${getAcriveStyle(index, 1)}`">
                {{ item2?.name }}
              </span>
            </span>
        </li>
...
function getAcriveStyle(index: number, type: 0 | 1) {
  ...
  return isActive ? 'bg-red/20 font-bold' : '';
}
</script>
