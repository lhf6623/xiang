<template>
  <div class="relative flex flex-col items-center w-324px h-400px bg-#eed3b3 select-none cursor-pointer">
    <NumberList :list="numbers" />
    <div class="relative w-324px h-360px">
      <svg class="absolute inset-0" width="324" height="360" viewBox="0 0 324 360">
        <!-- 外框2：最外层装饰框，距棋盘3px -->
        <rect :x="verticalX[0]-6" :y="horizontalLines[0]-6" :width="verticalX[8]-verticalX[0]+12" :height="horizontalLines[9]-horizontalLines[0]+12" fill="none" stroke="#000" stroke-width="2" />
        <!-- 外框1：棋盘边框（= 最外层网格线） -->
        <rect :x="verticalX[0]" :y="horizontalLines[0]" :width="verticalX[8]-verticalX[0]" :height="horizontalLines[9]-horizontalLines[0]" fill="none" stroke="#000" stroke-width="1" />

        <!-- 横线（内层8条，端点延长0.5px避免缺口） -->
        <line
          v-for="y in horizontalLines.slice(1, -1)" :key="'h-'+y"
          :x1="verticalX[0]-0.5" :x2="verticalX[8]+0.5" :y1="y" :y2="y" stroke="#000" stroke-width="1"
        />
        ...
        <!-- 斜线：九宫格 -->
        <line v-for="(d, i) in palaceDiagonals" :key="'d-'+i"
          :x1="d.x1" :y1="d.y1" :x2="d.x2" :y2="d.y2" stroke="#000" stroke-width="1"
        />

        <!-- 楚河汉界 -->
        <text x="162" y="182" text-anchor="middle" dominant-baseline="central" font-size="4" fill="#000">
          楚河　　　　　　　汉界
        </text>

        <!-- 星位标记 -->
        <path
          v-for="(s, i) in starPaths" :key="'s-'+i"
          :d="s" fill="none" stroke="#000" stroke-width="1" stroke-linecap="square" stroke-linejoin="round"
        />
      </svg>
